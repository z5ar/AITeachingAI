...
// 只保留小节完成状态和学习报告
function markSectionCompleted(sectionId, report) {
  const completed_at = new Date().toISOString();
  localStorage.setItem('ata_completed_' + sectionId, completed_at);
  localStorage.setItem('ata_report_' + sectionId, JSON.stringify(report));
}
// 简化后的进度变量（旧逻辑已弃用，但渲染代码仍引用）
let perItemProgress = [];
let timeSpentArr = [];
function saveProgressDraft(){ /* no-op after simplification */ }
function updateProgressInfo(){ /* minimal display */ try{ const info = el('progress-info'); if (info){ info.textContent = '' } }catch(e){} }
function getSectionCompleted(sectionId) {
  return localStorage.getItem('ata_completed_' + sectionId);
}
function getSectionReport(sectionId) {
  const r = localStorage.getItem('ata_report_' + sectionId);
...
// 全局对话历史容器
window.teacherHistory = window.teacherHistory || []
// 题目判题历史（用于学习报告）：仅记录选择/填空/简答的判题结果
window.judgeHistory = window.judgeHistory || []

// 学习时长（仅统计窗口处于焦点且页面可见的时间），按用户+小节持久化
let learnTimerId = null
let learnLastStart = null
let learnAccumMs = 0
let learnKey = null
function learnFormat(ms){
...
function getLearnKey(){
  const u = (currentUsername||'guest')
  const sid = (currentSection && currentSection.id) ? currentSection.id : 'none'
  return `ata_learn_time_${u}_${sid}`
}
function loadLearnTime(){
  try{
    learnKey = getLearnKey()
    const v = localStorage.getItem(learnKey)
    learnAccumMs = v ? parseInt(v,10)||0 : 0
  }catch(_){ learnAccumMs = 0 }
  updateLearnTimeUI()
}
function saveLearnTime(){
  try{ if (learnKey) localStorage.setItem(learnKey, String(learnAccumMs)) }catch(_){/* ignore */}
}
function updateLearnTimeUI(){
  const elTime = document.getElementById('learn-time')
  if (!elTime) return
  let total = learnAccumMs
  if (learnLastStart){ total += Date.now()-learnLastStart }
  elTime.textContent = `学习时长：${learnFormat(total)}`
}
function startLearnTimer(){
  // 仅在有当前小节、页面可见且窗口有焦点时计时
  if (!currentSection) return
  if (document.visibilityState !== 'visible') return
  if (!document.hasFocus()) return
  if (!learnLastStart) learnLastStart = Date.now()
  if (learnTimerId) return
  learnTimerId = setInterval(()=>{
    updateLearnTimeUI()
    // 周期性持久化，避免频繁写入
    if (learnLastStart && Math.random() < 0.25){ // 约每4秒一次
      saveLearnTime()
    }
  }, 1000)
}
function stopLearnTimer(){
  if (learnTimerId){ clearInterval(learnTimerId); learnTimerId = null }
  if (learnLastStart){
    learnAccumMs += Date.now()-learnLastStart
    learnLastStart = null
    saveLearnTime()
  }
  updateLearnTimeUI()
}
function resetLearnTimerState(){
  stopLearnTimer()
  learnAccumMs = 0
  learnLastStart = null
  learnKey = getLearnKey()
...
function openHistoryModal(){
  ...
  if (list){
    list.innerHTML = ''
    if (!window.teacherHistory || !window.teacherHistory.length){
      list.textContent = '暂无历史记录';
    } else {
      const ul = document.createElement('ul')
      ul.style.listStyle = 'none'; ul.style.padding = '0'; ul.style.margin = '0';
      window.teacherHistory.forEach((step)=>{
        const li = document.createElement('li')
        li.style.borderBottom = '1px solid #eee'; li.style.padding = '8px 4px';
        const type = step.type || 'text'
        if (type === 'text' || type === 'video'){
          li.textContent = `[${type}] ${step.content || ''}`
        } else if (type === 'problem'){
          const content = (typeof step.display === 'string' && step.display)
            || (typeof step.problem === 'string' && step.problem)
            || ''
          li.textContent = `[problem] ${content || '题目'}`
        } else {
          li.textContent = `[${type}] ${step.content || ''}`
        }
        ul.appendChild(li)
  ...
  if (closeBtn){ closeBtn.onclick = ()=>{ modal.classList.add('hidden'); modal.style.display='none' } }
  if (clearBtn){ clearBtn.onclick = ()=>{ window.teacherHistory = []; if (list) list.textContent = '已清空'; } }
}
...
async function selectSection(section){
  ...
  const modalSectionName = document.getElementById('section-modal-sectionname');
  const modalContent = document.getElementById('section-modal-content');
  const modalConfirm = document.getElementById('section-modal-confirm');
  const modalCancel = document.getElementById('section-modal-cancel');
  modalTitle.textContent = '进入学习';
  modalSectionName.textContent = section.title || '未命名小节';
  ...
  modalConfirm.onclick = async () => {
    ...
    modalContent.textContent = '正在加载...';
    try {
      const res = await apiPost(`/lesson/teacher/startSection/${section.id}`, {});
      if (res && res.status && res.status.code === 0 && res.first_step) {
        // 隐藏模态对话框
        ...
        try { window.judgeHistory = [] } catch(_){/* ignore */}
        try { loadLearnTime(); startLearnTimer(); } catch(_){/* ignore */}
        // 显示到主对话区
    ...
    problemAnswerCache = {};
    window.problemDraft = {};
  // 记录当前步骤对象用于答疑的 context
  try { window._currentItem = res.first_step } catch(_){/* ignore */}
  renderItem(res.first_step, dialogueContent);
    try {
      window.teacherHistory = window.teacherHistory || [];
      if (res.first_step && res.first_step.type !== 'problem') {
        window.teacherHistory.push(res.first_step)
      }
    } catch(e){}
        
      } else {
        modalContent.textContent = '未获取到内容';
      }
    } catch(e) {
      modalContent.textContent = '请求失败';
    }
...
function showJudgeResult(result, container) {
  ...
  // 处理 PPT 展示：若存在 item.ppt，则显示到对话区上方的 ppt-holder 中
  try {
    const holder = document.getElementById('ppt-holder');
    const img = document.getElementById('ppt-image');
    let pptUrl = null;
    if (item && typeof item.ppt === 'string') pptUrl = item.ppt;
    else if (item && Array.isArray(item.ppt) && item.ppt.length) pptUrl = item.ppt[0];
    if (pptUrl && holder && img) {
      img.src = pptUrl;
    ...
    apiPost('/problem/get', {problem_id: [problemId]}).then(data => {
      if (data && data.content && Array.isArray(data.content) && data.content[0]) {
        ...
        if (prob && typeof prob === 'object' && prob.ptype && prob.problem) {
          ...
          try {
            // 在历史中记录题干文本
            if (window.teacherHistory && Array.isArray(window.teacherHistory)) {
              window.teacherHistory.push({ type: 'problem', id: problemId, display: prob.problem })
            }
          } catch(e) { /* ignore */ }
        } else {
...
function renderProblemUI(prob, container, problemId, cache) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (res && Array.isArray(res) && res[0] && res[0].result) {
          ...
          showJudgeResult(r, container);
          submitted = true;
          btn.disabled = true;
          btn.textContent = '已提交';
          problemAnswerCache[paraIndex] = { answer: selected.slice(), submitted: true, result: r };
          // 提交后标记为100%
          perItemProgress[paraIndex] = 100;
          // draft保存答案和评分结果
          window.problemDraft = window.problemDraft || {}
          window.problemDraft[paraIndex] = { answer: selected.slice(), submitted: true, result: r };
          saveProgressDraft();
          updateProgressInfo();
          try { window.judgeHistory.push({ type:'choice', problem_id: problemId, problem: stem, answer: selected.slice(), score: r.score, mscore: r.mscore, std: r.std }) } catch(_){}
        } else {
          btn.disabled = false;
          btn.textContent = '提交';
          alert('判分失败');
  ...
  if (ptype === 'brief_response') {
          // 简答题：调用教师判题接口
          const stdAns = (arms && (arms.std || arms.answer || arms.std_answer)) ? (arms.std || arms.answer || arms.std_answer) : null;
          const mscoreVal = (typeof prob.mscore === 'number') ? prob.mscore : (typeof arms?.mscore === 'number' ? arms.mscore : 100);
          const reqBody = { problem: stem, std: stdAns, ans: val, mscore: mscoreVal };
          const r = await apiPost('/lesson/teacher/judge', reqBody);
          if (r && r.status && typeof r.score === 'number') {
            const result = { std: stdAns || undefined, score: r.score, mscore: mscoreVal, comment: r.comment };
            showJudgeResult(result, container);
            submitted = true;
            btn.disabled = true;
            btn.textContent = '已提交';
            input.disabled = true;
            // 缓存提交状态和结果
            problemAnswerCache[paraIndex] = { answer: val, submitted: true, result };
            window.problemDraft = window.problemDraft || {};
            window.problemDraft[paraIndex] = { answer: val, submitted: true, result };
            saveProgressDraft();
            try { window.judgeHistory.push({ type:'brief_response', problem: stem, answer: val, score: result.score, mscore: result.mscore, std: result.std, comment: result.comment }) } catch(_){}
          } else {
            btn.disabled = false;
            btn.textContent = '提交';
            alert('判分失败');
          ...
          const res = await apiPost('/problem/judge', judgeReq);
          if (res && Array.isArray(res) && res[0] && res[0].result) {
            const r0 = res[0].result;
            showJudgeResult(r0, container);
            submitted = true;
            btn.disabled = true;
            btn.textContent = '已提交';
            input.disabled = true;
            // 缓存提交状态和结果
            problemAnswerCache[paraIndex] = { answer: val, submitted: true, result: r0 };
            // draft保存答案和评分结果
            window.problemDraft = window.problemDraft || {};
            window.problemDraft[paraIndex] = { answer: val, submitted: true, result: r0 };
            saveProgressDraft();
            try { window.judgeHistory.push({ type:'blank_filling', problem_id: problemId, problem: stem, answer: val, score: r0.score, mscore: r0.mscore, std: r0.std }) } catch(_){}
          } else {
            btn.disabled = false;
            btn.textContent = '提交';
            alert('判分失败');
...
async function saveProgress(){
  if (!currentSection) return alert('先选小节');
  // 生成学习报告（可自定义结构，这里用对话流快照）
  const report = { items: dialogueItems, completed_at: new Date().toISOString() };
  markSectionCompleted(currentSection.id, report);
  // 上传到后端
  const payload = {
    completed_at: report.completed_at,
    draft: JSON.stringify(report)
  };
  const r = await apiPost(`/lesson/progress/${currentSection.id}/set`, payload);
  if (r.status && r.status.code === 0) {
    el('progress-info').textContent = `已保存（完成）`;
  } else if (r.status && r.status.code === 1) {
...
function bind(){
  ...
  // 学习时长：页面焦点/可见性事件
  window.addEventListener('focus', ()=>{ try{ startLearnTimer() }catch(_){}})
  window.addEventListener('blur', ()=>{ try{ stopLearnTimer() }catch(_){}})
  document.addEventListener('visibilitychange', ()=>{
    if (document.visibilityState === 'visible') { try{ startLearnTimer() }catch(_){}}
    else { try{ stopLearnTimer() }catch(_){} }
  })
  // 初始UI
  try{ updateLearnTimeUI() }catch(_){}
  ...
  if (nextBtn) nextBtn.addEventListener('click', async ()=>{
    try {
      const res = await apiPost('/lesson/teacher/next_step', {});
      if (res && res.status && res.status.code === 0 && res.content) {
        if (res.content.type === 'report') {
          // 收集学习报告数据并请求生成
          const reportModal = document.getElementById('report-modal');
          const reportContent = document.getElementById('report-modal-content');
          let totalMs = 0;
          try { totalMs = (learnAccumMs || 0) + (learnLastStart ? (Date.now()-learnLastStart) : 0) } catch(_){ totalMs = 0 }
          const payload = { time_spent: Math.floor(totalMs/1000), problems: (window.judgeHistory || []) };
          try {
            const gen = await apiPost('/lesson/teacher/report', payload);
            const text = (gen && gen.status && gen.content) ? gen.content : res.content;
            let display = text;
            ...
            reportContent.textContent = display;
          } catch(_){
            let fallback = res.content;
            if (typeof fallback !== 'string') fallback = JSON.stringify(fallback, null, 2);
            if (fallback.includes('\\n')) fallback = fallback.replace(/\\n/g, '\n');
            reportContent.textContent = fallback;
          }
          reportModal.classList.remove('hidden');
          reportModal.style.display = 'flex';
          const reportClose = document.getElementById('report-modal-close');
          if (reportClose) reportClose.onclick = () => {
            ...
            paraIndex = 0;
            const dialogueContent = document.getElementById('dialogue-content');
            if (dialogueContent) dialogueContent.textContent = '已退出小节';
            const nextBtn2 = document.getElementById('next-para');
            if (nextBtn2) nextBtn2.disabled = true;
            // 停止学习时长计时
            try { stopLearnTimer() } catch(_){/* ignore */}
            // 清空本小节判题历史
            try { window.judgeHistory = [] } catch(_){/* ignore */}
          };
        } else {
          // 正常渲染：使用统一渲染函数以支持 video/problem 等
          const dialogueContent = document.getElementById('dialogue-content');
          if (dialogueContent) dialogueContent.innerHTML = '';
          // 清空上一题提交缓存，避免状态残留
          problemAnswerCache = {};
          window.problemDraft = {};
          // 记录当前步骤对象用于答疑的 context
          try { window._currentItem = res.content } catch(_){/* ignore */}
          renderItem(res.content, dialogueContent);
          try {
            window.teacherHistory = window.teacherHistory || [];
            if (res.content && res.content.type !== 'problem') {
              window.teacherHistory.push(res.content)
            }
          } catch(e){}
        }
      } else {
  ...
  // 对话历史按钮（全局绑定）
  const histBtn = el('btn-history')
  if (histBtn) histBtn.addEventListener('click', openHistoryModal)
  // 答疑按钮
  const qaBtn = el('btn-qa')
  if (qaBtn) qaBtn.addEventListener('click', ()=>{
    const modal = document.getElementById('qa-modal');
    const input = document.getElementById('qa-input');
    const ansDiv = document.getElementById('qa-answer');
    if (input) input.value='';
    if (ansDiv){ ansDiv.style.display='none'; ansDiv.textContent=''; }
    if (modal){ modal.classList.remove('hidden'); modal.style.display='flex'; }
  })
  const qaCancel = document.getElementById('qa-cancel');
  if (qaCancel) qaCancel.onclick = ()=>{
    const modal = document.getElementById('qa-modal'); if (modal){ modal.classList.add('hidden'); modal.style.display='none'; }
  }
  const qaSubmit = document.getElementById('qa-submit');
  if (qaSubmit) qaSubmit.onclick = async ()=>{
    const input = document.getElementById('qa-input');
    const ansDiv = document.getElementById('qa-answer');
    if (!input) return;
    const q = input.value.trim();
    if (!q){ alert('请输入问题'); return; }
    qaSubmit.disabled = true; qaSubmit.textContent = '提交中...';
    try {
      // 当前主对话区显示的对象 index: 直接使用 paraIndex 或通过 teacherHistory? 需求说明为 object 的 idx 字段，若脚本项有 idx 则优先，否则用 paraIndex
      let ctxIdx = paraIndex;
      try {
        const dialogueContent = document.getElementById('dialogue-content');
        // 若当前项存储在 window._currentItem 且有 idx，可使用
        if (window._currentItem && typeof window._currentItem.idx === 'number') ctxIdx = window._currentItem.idx;
      } catch(_){/* ignore */}
      const resp = await apiPost('/lesson/teacher/answer', { problem: q, context: ctxIdx });
      let answer = (resp && resp.content) ? resp.content : '无回答';
      if (typeof answer !== 'string') answer = JSON.stringify(answer, null, 2);
      if (ansDiv){ ansDiv.textContent = answer; ansDiv.style.display='block'; }
      // 记录到对话历史
      try { window.teacherHistory.push({ type:'qa', question:q, answer }) } catch(_){/* ignore */}
    } catch(e) {
      if (ansDiv){ ansDiv.textContent = '请求失败'; ansDiv.style.display='block'; }
    } finally {
      qaSubmit.disabled = false; qaSubmit.textContent = '提交';
    }
  }
